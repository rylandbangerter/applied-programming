...
def fetch_and_clean_player_data(player_name, selected_stat):
    ...
    # --- Drop rows where 'ab' (at-bats) is 0 ---
    if 'ab' in df.columns:
...
if __name__ == "__main__":
    ...
    important_selected_stat = pd.Series(
        importances, index=selected_stat_features).sort_values(ascending=False)

    print(f"\nTop Features for {
          selected_stat} Prediction:\n", important_selected_stat.head(10))
